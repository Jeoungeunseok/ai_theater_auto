...
- [ ] GPT로 **팡이 Ep.01 스크립트** 생성 — 예: "상사 몰래 쉬는 법"
  - 포맷 공식: ① 후킹(도발적 본심 선언, 3초) ② 본심 수신(안테나 번쩍, 2초) ③ 꿀팁 3단(능청 코칭+과장 개그, 20초) ④ 마무리(공범 윙크+다음 본심 투표, 5초)
  - 프롬프트 v1 확정 (능청·죄책감0·훈계금지 톤 규칙 포함)
- [ ] Edge-TTS 보이스 생성 (한국어 화자 비교: ko-KR-InJoon/SunHi 등 — 능청 톤에 맞는 화자 선정)
- [ ] **퍼펫 클립 조립**: 감정 비트(심술→자신감→뿌듯) 순서로 클립 + 배경 + TTS + 자막 합성 (`render_short.py` ← 코어)
- [ ] FFmpeg `drawtext` 자막 스타일 확정 (숏폼 가독성)
- [ ] **수동 1편 완성** → 업로드해 톤·길이·자막·후킹 검증
